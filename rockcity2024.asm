...
PTVER:    EQU     '3'    ; パッチバージョン
DSTOCK    EQU     7      ; デフォルト自機数（最大9機）
          ORG     08400H ; 開始アドレス（限界まで削った）
;
;---- SCREEN & COLOR SET ----
;
...
;----------------------------------------------
; SUBDIV_CORE (UNROLLED / FAST VERSION)
; 入力: B=分子, DE=除数
; 出力: A=投影座標
; DJNZを分解して高速化
;----------------------------------------------
...
STR_LPF:
        ...
        JR      STR_LPF
;==============================================================================
; DRAW_CHAR - 色付きフォントを描画する (背景黒・高速版)
;==============================================================================
DRAW_CHAR:
        ; --- 0. 共通設定（8x8ドット固定） ---
        LD      HL, 8
        ...
        ; --- 1. ワークエリアを文字の色で塗りつぶす (HMMV) ---
        ; 1016ライン目などの見えない場所を「地の色」にする
        CALL    WAIT_VDP
        LD      HL, 0
        LD      (V_DX), HL
        LD      HL, 1016        ; ワークエリアのY座標
        LD      (V_DY), HL

        ; 指定された色(CUR_CLR)をVDP形式（1バイト内に2ドット分）に変換
        LD      A, (CUR_CLR)
        RLCA
        RLCA
        RLCA
        ...
        AND     0FH
        OR      B
        LD      (V_CLR), A      ; 書き込み色をセット

        LD      A, 0C0H         ; HMMVコマンド (VRAM高速塗りつぶし)
        LD      (V_CMD), A
        CALL    SEND_VCMD       ; 実行

        ; --- 2. フォントをANDで重ねて「形」を切り抜く (LMMM + AND) ---
        ; 白いフォントがある部分だけ、先ほどの色が残る
        CALL    WAIT_VDP
        
        ; フォントソース座標の計算 (CUR_CHR から算出)
        LD      A, (CUR_CHR)
        AND     0FH
        ADD     A, A
        ADD     A, A
        ...
        LD      (V_DX), HL
        LD      HL, 1016
        LD      (V_DY), HL      ; 転送先はワークエリア
        
        LD      A, 091H         ; LMMMコマンド + AND演算
        LD      (V_CMD), A
        CALL    SEND_VCMD       ; 実行

        ; --- 3. ワークエリアから画面へ高速コピー (HMMM) ---
        ; 背景が黒なら、透過処理(TIMP)を使わずにHMMMで上書きするのが最速
        CALL    WAIT_VDP
        LD      HL, 0
        LD      (V_SX), HL      ; ワークエリアのX=0
        LD      HL, 1016
        LD      (V_SY), HL      ; ワークエリアのY=1016
        
        ; 描画先の座標(CUR_X, CUR_Y)をセット
        LD      A, (CUR_X)
        LD      L, A
        LD      H, 0
        LD      (V_DX), HL
        ...
        CP      1
        JR      Z, PAGE0
        LD      DE, 256         ; ページ1ならY座標に256加算
        ADD     HL, DE
PAGE0:
        LD      (V_DY), HL
        
        ; ★ここが高速化の肝：098H(LMMM)を 0D0H(HMMM) に変更
        LD      A, 0D0H         ; HMMMコマンド (高速矩形転送)
        LD      (V_CMD), A
        CALL    SEND_VCMD       ; 実行
        RET
...
;==============================================================================
; FT_EXPAND_256 - 太字化してPage 3 (18000H) へ展開
;==============================================================================
FT_EXPAND_256:
        LD      A, 0
        LD      (WK_CHR), A
FT_LP:
        ; --- 1. 読み込み元 (CGROM) のアドレス計算 ---
        LD      A, (WK_CHR)
        LD      L, A
        LD      H, 0
        ADD     HL, HL          ; *2
        ADD     HL, HL          ; *4
        ADD     HL, HL          ; *8 (1文字8バイト)
        LD      DE, (CGPNT)     ; BIOSのフォント開始アドレス
        ADD     HL, DE
        
        ; --- 2. BIOSからフォントデータを 8バイト取得 ---
        LD      DE, FONT_TMP
        LD      B, 8
FT_GET:
        PUSH    BC
        ...
        PUSH    HL
        LD      A, (CSLOT)
        CALL    000CH           ; 異スロット(BIOS)から1バイト読み込み
        LD      C, A
        POP     HL
        POP     DE
        LD      A, C
        LD      (DE), A         ; ワーク(FONT_TMP)へ保存
        INC     DE
        INC     HL
        POP     BC
        DJNZ    FT_GET

        ; --- 3. VRAM Page 3 への書き込み準備 ---
        ; 展開先座標: X = (ID & 0Fh) * 8,  Y = (ID >> 4) * 8
        ; Page 3のアドレス = 18000H + (Y * 128) + (X / 2)
        
        LD      A, (WK_CHR)
        AND     0F0H            ; 上位4bit (0, 16, 32...)
        SRL     A
        SRL     A               ; A = (ID>>4) * 4
        ; Aは現在、文字単位のYオフセット。1文字8ラインなので A*2 = 最終的なY
...
FT_LINE:
        ...
        ; --- VDP レジスタ設定 (18000Hへの書き込み) ---
        DI
        LD      A, 6            ; 18000H = 1 10 0000 0000 0000b -> R#14は 06H
        OUT     (VDP_REG), A
        LD      A, 142          ; R#14
        OUT     (VDP_REG), A
        
        LD      A, L
        OUT     (VDP_REG), A    ; 下位8bit
        LD      A, H
        OR      40H             ; 書き込み指定
        OUT     (VDP_REG), A    ; 中位6bit
        
        ; --- 太字化ロジック ---
        LD      A, (IX)
        LD      C, A            ; C = 元
        SRL     A
        OR      C               ; A = 太字パターン (1bit右とOR)
        LD      C, A
        INC     IX
